  ...
  <v-app dark>
    <v-container>
      <v-row justify="center" class="ma-2 red--text text-center">
        <h1 v-if="isNotFound">ページが見つかりませんでした。</h1>
        <h1 v-else-if="isForbidden">アクセスできないページです。</h1>
      ...
      <v-row justify="center">
        <v-btn to="/" class="ma-5">トップページに戻る</v-btn>
        <v-btn class="ma-5" @click="$router.go(-1)">前の画面に戻る</v-btn>
      </v-row>
    </v-container>
  </v-app>
</template>
